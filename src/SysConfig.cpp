...
#include "Xml.h"
    ...
    bool SystemConfig::load(const char * config){
        ...
        if (settings){
            ...
            nod = 0;
            nod = settings->getNode(L"useLineTension");
            if (nod){
                wcscpy(tmp, nod->getValue());
                wcstombs(ctmp, tmp, 100);
                useLineTension = atoi(ctmp);
            }
...
//------------------------------------------
    bool SystemConfig::write(const char * config){
        ...
        XmlNode _lineTension;
        sprintf(buf, "%d", useLineTension);
        mbstowcs(wbuf, buf, 255);
        _lineTension.setName(L"useLineTension");
        _lineTension.setValue(wbuf);
        Settings.addChild(_lineTension);
